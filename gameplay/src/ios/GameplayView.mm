...
@interface GameplayView () <UIKeyInput>
{
    EAGLContext* context;
...
- (void)handleTapGesture:(UITapGestureRecognizer*)sender
{
    ...
    Platform::gestureTapEventInternal(location.x, location.y);
}
...
- (void)handleLongTapGesture:(UILongPressGestureRecognizer*)sender
{
    ...
    else if (sender.state == UIGestureRecognizerStateEnded)
    {
        ...
        Platform::gestureLongTapEventInternal(location.x, location.y, currentTimeStamp - _gestureLongTapStartTimestamp);
    }
}
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer*)sender
{
    ...
    CGPoint location = [sender locationInView:self];
    Platform::gesturePinchEventInternal(location.x, location.y, factor);
}
...
- (void)handleSwipeGesture:(UISwipeGestureRecognizer*)sender
{
    ...
    Platform::gestureSwipeEventInternal(location.x, location.y, gameplayDirection);
}
...
- (void)handleLongPressGestures:(UILongPressGestureRecognizer*)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateChanged)
    {
        if (_gestureDraging)
        {
            Platform::gestureDragEventInternal(location.x, location.y);
        }
        else
        {
            ...
            if (delta >= GESTURE_LONG_PRESS_DISTANCE_MIN)
            {
                ...
                Platform::gestureDragEventInternal(_gestureLongPressStartPosition.x, _gestureLongPressStartPosition.y);
            }
        }
    }
    ...
    if (sender.state == UIGestureRecognizerStateEnded)
    {
        if (_gestureDraging)
        {
            Platform::gestureDropEventInternal(location.x, location.y);
